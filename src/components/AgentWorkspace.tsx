...
export const AgentWorkspace: React.FC<AgentWorkspaceProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 text-white rounded-xl p-4 sm:p-8 shadow-xs border border-slate-800 relative overflow-hidden">
        <div className="relative z-10 max-w-3xl">
          ...
          <form onSubmit={handleSend} className="mt-4 sm:mt-6">
            <div className="relative flex items-center">
              <input
                ...
                disabled={isLoading}
                className="w-full py-3.5 pl-3.5 sm:pl-4 pr-20 sm:pr-32 text-sm text-white bg-slate-800/90 rounded-lg border border-slate-700 focus:border-slate-400 focus:ring-1 focus:ring-slate-400 outline-hidden transition-all placeholder:text-slate-500 shadow-inner min-h-[48px]"
              />
              <button
                id="agent-chat-submit-btn"
                type="submit"
                disabled={isLoading || !inputText.trim()}
                className="absolute right-1 px-3 sm:px-4 py-2 bg-white hover:bg-slate-100 text-slate-900 rounded-md text-xs sm:text-sm font-bold transition-all disabled:opacity-40 flex items-center gap-1.5 shadow-xs cursor-pointer min-h-[40px] touch-manipulation"
              >
                {isLoading ? (
                  <RefreshCw className="w-4 h-4 animate-spin text-slate-900" />
                ) : (
                  <>
                    <span className="hidden xs:inline">Ask Agent</span>
          ...
          <div className="mt-3 sm:mt-4">
            <div className="flex items-center gap-1.5 overflow-x-auto pb-2 -mx-1 px-1 sm:mx-0 sm:px-0 sm:flex-wrap sm:overflow-visible">
              <span className="text-slate-400 text-[11px] sm:text-xs font-medium shrink-0">Try asking:</span>
              {quickPrompts.map((qp, idx) => (
                <button
                  key={idx}
                  onClick={() => handleQuickPrompt(qp)}
                  className="px-2.5 py-1.5 rounded-md bg-slate-800 hover:bg-slate-700 text-slate-300 text-[11px] sm:text-xs transition-colors border border-slate-700 shrink-0 whitespace-nowrap cursor-pointer min-h-[34px] flex items-center touch-manipulation active:bg-slate-600"
                >
                  "{qp}"
                </button>
              ))}
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-3.5 sm:p-5 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 pt-1">
            <span className="px-2.5 py-1 bg-slate-100 border border-slate-200 text-slate-800 rounded-md text-xs font-medium flex items-center gap-1">
              ...
              <span className="truncate max-w-[160px] sm:max-w-none">{currentCriteria.location || 'Any Location'}</span>
            </span>
        ...
        <div className="flex items-center gap-2 pt-1 sm:pt-0 shrink-0 w-full sm:w-auto">
          <button
            ...
            className="flex-1 sm:flex-initial px-3.5 py-2.5 sm:py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 rounded-lg sm:rounded-md text-xs font-medium transition-colors flex items-center justify-center gap-1.5 cursor-pointer min-h-[44px] sm:min-h-0 touch-manipulation"
          >
          ...
          {properties.length >= 2 && (
            <button
              ...
              className="flex-1 sm:flex-initial px-3.5 py-2.5 sm:py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg sm:rounded-md text-xs font-medium transition-colors flex items-center justify-center gap-1.5 cursor-pointer min-h-[44px] sm:min-h-0 touch-manipulation"
            >
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 sm:p-5 space-y-4 animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
            <div>
              ...
              <input
                ...
                className="w-full p-2.5 sm:p-2 bg-white border border-slate-300 rounded-md text-slate-900 text-sm sm:text-xs focus:border-slate-500 outline-hidden min-h-[42px] sm:min-h-0"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                step={25000}
                className="w-full p-2.5 sm:p-2 bg-white border border-slate-300 rounded-md text-slate-900 text-sm sm:text-xs font-mono focus:border-slate-500 outline-hidden min-h-[42px] sm:min-h-0"
              />
            </div>
          ...
          <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2 pt-3 border-t border-slate-200">
            <button
              ...
              className="w-full sm:w-auto px-3.5 py-2 text-xs font-medium text-slate-600 hover:bg-slate-200 rounded-md transition-colors min-h-[42px] sm:min-h-0"
            >
              Cancel
            </button>
            <button
              onClick={handleApplyManual}
              className="w-full sm:w-auto px-4 py-2 text-xs font-semibold bg-slate-900 text-white rounded-md hover:bg-slate-800 transition-colors shadow-xs min-h-[42px] sm:min-h-0"
            >
              Apply Changes
            </button>
          </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 text-xs text-slate-500 px-1">
        <div>
        ...
        <div className="flex items-center gap-1.5 text-slate-400">
          ...
          <span className="truncate">Real Coordinates & OpenStreetMap Amenities Verified</span>
        </div>
      </div>
    </div>
  );
