...
export const PropertyComparisonModal: React.FC<PropertyComparisonModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
      <div className="bg-white rounded-xl max-w-6xl w-full shadow-2xl border border-slate-200 overflow-hidden my-auto max-h-[92vh] flex flex-col">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
          <div className="flex items-center space-x-2">
            <GitCompare className="w-5 h-5 text-slate-700" />
            <h2 className="font-bold text-base sm:text-lg text-slate-900">
              Side-by-Side Property Comparison ({properties.length} Properties)
            </h2>
          </div>
          <button
            onClick={onClose}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wider">
                  <th className="py-3 px-4 w-40 bg-slate-50 rounded-tl-lg font-semibold">Specification</th>
                  {properties.map((p, idx) => (
                    <th key={p.id} className="py-3 px-4 bg-slate-50">
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {/* Asking Price */}
                <tr>
                  <td className="py-3 px-4 font-semibold text-slate-700 bg-slate-50/60">Asking Price</td>
                  {properties.map((p) => (
                ...
                {/* Match Score */}
                <tr>
                  <td className="py-3 px-4 font-semibold text-slate-700 bg-slate-50/60">Match Score</td>
                  {properties.map((p) => (
                ...
                {/* Price Per Sq Ft */}
                <tr>
                  <td className="py-3 px-4 font-semibold text-slate-700 bg-slate-50/60">Price / sqft</td>
                  {properties.map((p) => (
                ...
                {/* Bedrooms & Bathrooms */}
                <tr>
                  <td className="py-3 px-4 font-semibold text-slate-700 bg-slate-50/60">Beds / Baths</td>
                  {properties.map((p) => (
                ...
                {/* Total Area */}
                <tr>
                  <td className="py-3 px-4 font-semibold text-slate-700 bg-slate-50/60">Living Area</td>
                  {properties.map((p) => (
                ...
                {/* Location */}
                <tr>
                  <td className="py-3 px-4 font-semibold text-slate-700 bg-slate-50/60">Location</td>
                  {properties.map((p) => (
                    <td key={p.id} className="py-3 px-4 text-slate-800 text-xs">
                ...
                {/* Property Type */}
                <tr>
                  <td className="py-3 px-4 font-semibold text-slate-700 bg-slate-50/60">Type & Style</td>
                  {properties.map((p) => (
                    <td key={p.id} className="py-3 px-4 text-slate-800 text-xs">
                ...
                {/* Key Features */}
                <tr>
                  <td className="py-3 px-4 font-semibold text-slate-700 bg-slate-50/60">Features</td>
                  {properties.map((p) => (
                    <td key={p.id} className="py-3 px-4">
                ...
                {/* Pros */}
                <tr>
                  <td className="py-3 px-4 font-semibold text-slate-700 bg-slate-50/60">Key Advantages</td>
                  {properties.map((p) => (
                ...
                {/* Compromises / Cons */}
                <tr>
                  <td className="py-3 px-4 font-semibold text-slate-700 bg-slate-50/60">Tradeoffs</td>
                  {properties.map((p) => (
                ...
                {/* Select Action */}
                <tr>
                  <td className="py-3 px-4 bg-slate-50/60"></td>
                  {properties.map((p) => (
                    <td key={p.id} className="py-3 px-4">
                      <button
                        onClick={() => {
                        ...
                        className="w-full py-2 px-3 bg-slate-900 hover:bg-slate-800 text-white rounded-md text-xs font-semibold flex items-center justify-center space-x-1 transition-colors"
                      >
